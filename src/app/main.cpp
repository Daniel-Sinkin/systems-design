...
#include "random_utils.hpp" // IWYU pragma: keep
#include "spsc_queue.hpp"   // IWYU pragma: keep
...
[[nodiscard]] auto to_string(SymbolId symbol) -> std::string_view {
    switch (symbol) {
    case SymbolId::AAPL:
        return "AAPL";
    case SymbolId::MSFT:
...
struct Order {
    ...
    [[nodiscard]] auto is_sell() const noexcept -> bool { return !is_buy(); }
...
[[nodiscard]] auto to_string(Order::Action action) -> std::string_view {
    switch (action) {
...
[[nodiscard]] auto to_string(Response::Type response_type) -> std::string_view {
...
public:
    ...
    auto submit_order(IngressQueue &queue, Price price, Quantity qty, Order::Action action, Order::Flags flags) -> void {
        const OrderMessage msg{
            .connection_id = id_,
            .seq_num = 0, // We don't do TCP byte buffer handling so this is always 0
            .symbol = symbol_,
            .order = {
...
int main() {
    [[maybe_unused]] IngressQueue ingress_queue;
    ...
    enum UserType : u8 {
        Invalid = 0,
        Buyer,     // Opens Long positions
        Seller,    // Opens Short Positions
        Mixed,     // Opens Long and Short Positions
        Inactive,  // Doesn't open any positions
        Malformed, // Makes malformed requests which will fail
        Count,
    };
    struct UserTask {
        SymbolId symbol{SymbolId::Invalid};
    ...
    auto user_tasks = std::to_array<UserTask>({
    ...
    std::vector<std::jthread> user_tasks_threads{};
    user_tasks_threads.reserve(user_tasks.size());
    for (const auto &task : user_tasks) {
        user_tasks_threads.emplace_back(
            [&, task] {
                using namespace std::chrono_literals;
                User user{task.symbol};
                while (true) {
                    if (quit_signal.load(std::memory_order_relaxed)) [[unlikely]] {
                        break;
                    }
                    user.submit_order(
                        ingress_queue,
    ...
    std::jthread ingress_worker{[&] {
        ...
        while (true) {
            ...
            const auto send_reject = [&](const IngressErrorMessage &msg) {
                Response resp{
                    .type = Response::Type::Reject,
                    .connection_id = tmp.connection_id,
                    .client_order_id = tmp.order.client_order_id,
                };
            ...
            if (ingress_queue.pop(tmp)) [[likely]] {
                if (tmp.symbol == SymbolId::Invalid or tmp.symbol >= SymbolId::Count) [[unlikely]] {
                    send_reject(k_string_ingress_invalid_symbol);
                } else {
                    const auto idx = static_cast<usize>(std::to_underlying(tmp.symbol));
                    if (symbol_queues[idx].push(tmp.order)) [[likely]] {
                        log.log(std::format("Successfully enqueued an order for symbol '{}' there are now {} orders in there.", to_string(tmp.symbol), symbol_queues[idx].size()));
                    } else {
                        send_reject(k_string_ingress_full_queue);
                    }
                }
            }
        }
    ...
    std::vector<std::jthread> symbol_order_worker{};
    symbol_order_worker.reserve(SymbolCount);
    for (auto i = 0zu; i < SymbolCount; ++i) {
        symbol_order_worker.emplace_back(
            [&, i] {
                using namespace std::chrono_literals;
                const auto symbol = static_cast<SymbolId>(i);
    ...
    std::jthread event_queue_worker{
        [&] {
            using namespace std::chrono_literals;
            ...
            while (true) {
                ...
                Response tmp{};
                if (response_event_queue.pop(tmp)) [[likely]] {
                    log.log(std::format("Sending response to connection '{}' for order_id '{}' of type '{}' with message '{}'", tmp.connection_id, tmp.client_order_id, to_string(tmp.type), std::string_view(tmp.msg)));
                }
